...
def main():
    ...
    try:
        conn.execute("BEGIN")
        cursor.execute(
            "CREATE TABLE nodes (id INTEGER PRIMARY KEY NOT NULL, x FLOAT NOT NULL, y FLOAT NOT NULL, mass FLOAT NOT NULL)"
        )
        cursor.execute(
            "CREATE TABLE edges (source INTEGER NOT NULL, target INTEGER NOT NULL, weight FLOAT NOT NULL)"
        )
        ...
        nodes = [
            ...
            for id, p in zip(node_ids, low_embeddings, strict=False)
        ]
